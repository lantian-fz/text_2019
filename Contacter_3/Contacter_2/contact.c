...
void FilePrintfCapacity(Contact *con)
{
	assert(con);
	int i = 0;
	FILE* pf = fopen("Contacter.txt", "w");
	if (pf == NULL)
	{
		printf("保存通讯录失败！\n");
		return;
	...
	for (i = 0; i < con->sz; i++)
	{
		fprintf(pf, "%10s %10d %10s %15s %20s\n", con->data[i].name, con->data[i].age,
			con->data[i].sex, con->data[i].phone, con->data[i].addr);
	}	
...
void FileScanfCapacity(Contact *con)
{
	assert(con);
	int i = 0;
	FILE* pf = fopen("Contacter.txt", "r");
	//Person tmp = { 0 };
	if (pf == NULL)
	{
		printf("加载通讯录失败！\n");
	...
	while (AddCapacity(con), fscanf(pf, "%s %d %s %s %s", con->data[con->sz].name, &(con->data[con->sz].age),
		con->data[con->sz].sex, con->data[con->sz].phone, con->data[con->sz].addr)!=EOF)
	{
		//AddCapacity(con);
		con->sz++;
	}
